...
switch($_SERVER['REQUEST_METHOD']):
	case "POST":
		...
		if((int)$_SESSION['_lgatt'] + 1 > 5):
			
			$admin_data  = Admins::get("admin_email='{$admin_email}'");
			
			if(haveRows($admin_data)):
				Admins::set("admin_status",0,"admin_email='{$admin_email}'");
				$message = "USUARIO BLOQUEADO: Se detectaron 6 intentos fallidos de ingreso.";
			else:
				$message = "ACCESO BLOQUEADO: Se detectaron 6 intentos fallidos de ingreso.";
			endif;
				...
				if(!$login):
					...
					if(haveRows($admin_status)):
						...
						if(count($failed_attempts) > 5):
							...
							setLocked("USUARIO BLOQUEADO: Se detectaron 6 intentos fallidos de ingreso. (1)");
						endif;
					endif;
