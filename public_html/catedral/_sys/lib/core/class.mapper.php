...
class Mapper Extends Mysql{
	...
	private $rootemail = 'admin@cms.com';
	private $root_pass = 'Root@2020*';
	...
	public static function run($overwrite_files=false){
		...
		/**/
		/*crea tablas de administradores*/
		...
		if(is_array($check_admins) && count($check_admins) == 0):
			$admins_table = "CREATE TABLE `admins` (
			  ...
			  `admin_isroot` tinyint(1) NOT NULL DEFAULT '0',
		...
		$check_admin_root = "SELECT admin_email FROM admins WHERE admin_email = '" . $obj->rootemail . "'";
		...
		if(is_array($check_admin_root) && count($check_admin_root) == 0):
			...
			admin_email = '" . $obj->rootemail . "',
			admin_password = MD5('{$root_pass}_" . strtoupper(strrev($obj->rootemail))."'),
	...
	<li class="heading"><span>Contenidos</span></li>' . "\n";
		...
		if(is_array($tables) && count($tables) > 0):
			...
			foreach($tables as $table):
				if(!in_array($table['TABLE_NAME'], $override_tables)):
					...
					if(strlen($table['TABLE_COMMENT']) > 0):
						$table_config_per = @json_decode("{". $table['TABLE_COMMENT'] ."}");
			...
			if(file_exists(pathToView . "admins.form.php")):
				@unlink(pathToView . "admins.form.php");
			endif;

			if(!is_file( pathToView . "admins.form.php" )):
				$file_admin_form = @fopen( pathToView . "admins.form.php", "w+" );
			endif;
	...
	endforeach;
		...
		if(strlen($_FILES[\'{$field_file_name}\'][\'name\'])> 0):
			...
			if($id > 0):
				...
				if($table['TABLE_NAME'] != "admins" && $table['TABLE_NAME'] != "admin_login_attempts" && $table_menu):
...
?>
		...
		$content_form = '
<?php require_once(\'_sys/init.php\');?>
	...
	foreach($columns as $column):
		...
		if(strlen(trim($column['COLUMN_COMMENT'])) > 0):
			...
			if($column_config instanceof stdClass):
				...
				$field_crop 		 = isset($column_config->crop) ? (strtolower($column_config->crop) == "true" ? true : false) : false;

				if($field_crop):
					$crop_script 	= '
						<a href="" class="btn-action glyphicons scissors btn-primary" onclick="module(\'crop&module=' . $table['TABLE_NAME'] . '&image_id=<?php echo $list[\'' . $primaryKey . '\'];?>&image_big_url=<?php echo $list[\''.$colprefix.'_image_big_url\'];?>\');return!1;"><i></i></a>';
				endif;
...
'.$onlist_cols_row.'
				...
				<td class="center" style="width: 120px;">'.$crop_script.'
...
$goglemaps_script 		= null;
$datepicker_script 		= null;
$colorpicker_script 	= null;
$crop_script 			= null;
...
if(!Captcha::validate(param("'.$colprefix.'_captcha"))):
	...
	if(param("token") == token("'.$table['TABLE_NAME'].'")):
		...
		if(!isValidEmail(param(\''.$colprefix.'_email\'))):
			$error = array(\'usuario_email\' => \'Por favor, escribe una dirección de correo válida.\' );
		endif;
...
?>';
						...
						*****************************************************
						*/
						$filepath = "_sys/templates/".$table['TABLE_NAME']."_template.html";
