...
function credito_clientes($cedula, $enlace){
    $data = array(
		"cedula" => $cedula,
		"ruc" => ""
	);
...
function listar_depositos($cedula, $enlace){
    $data = array(
		"DEPO_CODIGO" => $cedula
	);
	$session = curl_init($enlace);
    ...
    $response = json_decode($session_response);
	//$atencion   = $response->ATENCION_DELI;
	//$horario = $response->HORARIOS_DELI;
	return array($response);
	//return "f";
}
...
function getGeocodeData($address) { 
    ...
    $googleMapUrl = "https://maps.googleapis.com/maps/api/geocode/json?address={$address}&key=AIzaSyDckW-N1NSxLfBWeOQ874Sfu5w3zNtuoOs&callback=moodjs.MapApiLoaded";
