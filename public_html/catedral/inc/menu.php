<ul class="menu">
  <?
    foreach ($categorias as  $key => $catmenu) {
      echo '<li class="list">
              <a href="productos/'.$catmenu["categoria_slugit"].'">'.$catmenu["categoria_nombre"].'</a>';
              ...
              if(haveRows($subcategoriaNivel1)){
                echo '<ul class="items">';
                  foreach ($subcategoriaNivel1 as $nivel1) {
                      ...
                      echo '<li '.$classList.'> <a href="productos/'.$nivel1["categoria_slugit"].'" >'.$nivel1["categoria_nombre"].'</a>';
...
for (i = 0; i < list.length; i++) {
    ...
    list[i].addEventListener('mouseleave', accordion_remove);
}
</script>
<?