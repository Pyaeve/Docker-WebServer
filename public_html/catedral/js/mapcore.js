...
function placeMarker(){
	...
	if(marker_object == null && !marker_waiting){
		...
		google.maps.event.addListener(drawingManager, 'markercomplete', function(marker){
			...
			if(point){
				var ltln = point.split(',');
				//var direccion = document.getElementById("direccion");
				//$('.Lat').val(ltln[0]);
				//$('.Lng').val(ltln[1]);
				console.log('.Lng');
				console.log($('.Lng').val());
				var url = 'https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat='+ltln[0]+'&lon='+ltln[1].trim();
				
				//var url = 'https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=-25.306760656133985&lon=-57.640549156623884';
				$.get(url, function(data){
					$('.direccion').val(data.address.road);
					$('.ciudad').val(data.address.city);
					if(data.address.suburb){
						$('.barrio').val(data.address.suburb);
					}else if(data.address.neighbourhood){
						$('.barrio').val(data.address.neighbourhood);
					}
